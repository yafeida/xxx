...
public class POIUtils {
    ...
    static void writeByLocalOrBrowser(HttpServletResponse response, String fileName, SXSSFWorkbook wb, OutputStream out) {
        try {
            ...
            if (response != null) {
                ...
                response.setContentType(MonkeyConstant.XLSX_CONTENT_TYPE);
                response.setHeader("Content-disposition", "attachment; filename="
                        + URLEncoder.encode(String.format("%s%s", fileName, MonkeyConstant.XLSX_SUFFIX), "UTF-8"));
                if (out == null) {
    ...
    private static void checkExcelFile(String file) {
        if (!file.endsWith(MonkeyConstant.XLSX_SUFFIX)) {
