...
var $rolesSelect = $addForm.find("select[name='rolesSelect']");
var $roles = $addForm.find("input[name='roles']");
...
$(function () {
    ...
    $("#user-add .btn-save").click(function () {
        ...
        if (flag) {
            if (name === "save") {
                $.post(ctx + "user/add", $addForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
            ...
            if (name === "update") {
                $.post(ctx + "user/update", $addForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
                        $MB.refreshTable("userTable");
...
function closeModal() {
    ...
    $addForm.find("input[name='username']").removeAttr("readonly");
    $addForm.find(".user_password").show();
    $addForm.find("input[name='status']").prop("checked", true);
